...
@property (weak, nonatomic) IBOutlet UITableViewCell *OTAUpgradeCell;
@property (weak, nonatomic) IBOutlet UITableViewCell *TimelapseCell;
...
@property (weak, nonatomic) IBOutlet UILabel *parkingModeL;
@property (weak, nonatomic) IBOutlet UISwitch *parkingModeSwitch;
@property (weak, nonatomic) IBOutlet UISegmentedControl *parkingModeSeg;
...
@property (weak, nonatomic) IBOutlet UILabel *upgradeL;
@property (weak, nonatomic) IBOutlet UILabel *timeLapseL;
...
@property (nonatomic,assign) int formatTime;
...
- (void)viewDidLoad {
    ...
    self.parkingModeL.text = NSLocalizedString(@"语言设置", nil);
    self.upgradeL.text = NSLocalizedString(@"OTA升级", nil);
    self.timeLapseL.text = NSLocalizedString(@"缩时录影", nil);
...
//    [self.clipDurationSegment setTitle:NSLocalizedString(@"5分钟", nil) forSegmentAtIndex:2];
    ...
    [self.gSensorSegment setTitle:NSLocalizedString(@"高", nil) forSegmentAtIndex:3];
    [self.gSensorSegment setTitle:NSLocalizedString(@"中", nil) forSegmentAtIndex:2];
    [self.gSensorSegment setTitle:NSLocalizedString(@"低", nil) forSegmentAtIndex:1];
    ...
    [self.parkingModeSeg setTitle:NSLocalizedString(@"德语", nil) forSegmentAtIndex:0];//把停车监控替换成音量
    [self.parkingModeSeg setTitle:NSLocalizedString(@"英语", nil) forSegmentAtIndex:1];
...
- (void)updateUIWithSettingInfo{
    ...
    if (settingInfo) {
        ...
        self.parkingModeSeg.selectedSegmentIndex = settingInfo.parkingMode;
        ...
        self.parkingModeSeg.selectedSegmentIndex = UISegmentedControlNoSegment;
...
//                [APKAlertTool showAlertInViewController:self title:nil message:NSLocalizedString(@"检测SD卡失败", nil) confirmHandler:nil];
//                return;
//            }
//
//            [self formatSdCard];
...
//        }];
    ...
    }else if (cell == self.OTAUpgradeCell){
        
        MStarUpgradeHandler *handler = [[MStarUpgradeHandler alloc] init];
                
        MBProgressHUD *HUD = [MBProgressHUD showHUDAddedTo:self.navigationController.view animated:YES];
        HUD.label.text = NSLocalizedString(@"升级包正在下载，请稍后...", nil);
        self.HUD = HUD;
...
-(void)excuteOTAUpgrate
{
    ...
    NSString *model = [versonStr containsString:@"DC1"] ? @"Ampire_DC1" : @"Ampire_DC2";
    __weak typeof (self) weakSelf = self;
    ...
    NSURLSessionDownloadTask *downloadTask = [manager downloadTaskWithRequest:request2 progress:nil destination:^NSURL *(NSURL *targetPath, NSURLResponse *response) {
        ...
        if (error) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                weakSelf.HUD.label.text = NSLocalizedString(@"升级包上传中...", nil);
            });
            ...
            [handler threadSafeUpgradeWithFilePath:[filePath path] cb:^(BOOL success, NSString *errorInfo) {
                ...
                [APKAlertTool showAlertInViewController:weakSelf title:nil message:NSLocalizedString(@"升级包已经上传成功，是否立即重启？", nil) cancelHandler:^(UIAlertAction *action) {
                    nil;
...
- (IBAction)toggleTimelapseSwitch:(UISwitch *)sender {
    ...
    [[APKDVRCommandFactory setCommandWithProperty:property value:value] execute:^(id responseObject) {
        ...
        [APKDVR sharedInstance].settingInfo.timelapse = sender.isOn;
        [hud hideAnimated:YES];
        [APKAlertTool showAlertInView:self.view andText:NSLocalizedString(@"设置成功", nil)];
        ...
        [hud hideAnimated:YES];
        if (rval == 701) {
            [APKAlertTool showAlertInViewController:weakSelf title:nil message:NSLocalizedString(@"没有ACC，无法设置此功能", nil) confirmHandler:^(UIAlertAction *action) {
                sender.on = !sender.on;
            }];
        }
        [APKAlertTool showAlertInViewController:weakSelf title:nil message:NSLocalizedString(@"设置失败！", nil) confirmHandler:^(UIAlertAction *action) {
...
- (IBAction)updateParkingModeSeg:(UISegmentedControl *)sender {
    
    if (!self.isSettable) return;
    
    MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:self.navigationController.view animated:YES];
    
    NSString *value = [APKDVR sharedInstance].settingInfo.parkingModeMap[sender.selectedSegmentIndex];
    NSString *property = @"Camera.Menu.ApkLanguage";
    __weak typeof(self)weakSelf = self;
    [[APKDVRCommandFactory setCommandWithProperty:property value:value] execute:^(id responseObject) {
        
        [APKDVR sharedInstance].settingInfo.parkingMode = sender.selectedSegmentIndex;
        [hud hideAnimated:YES];
        [APKAlertTool showAlertInView:self.view andText:NSLocalizedString(@"设置成功", nil)];
        ...
        [APKAlertTool showAlertInViewController:weakSelf title:nil message:NSLocalizedString(@"设置失败！", nil) confirmHandler:^(UIAlertAction *action) {
            ...
            sender.selectedSegmentIndex = [APKDVR sharedInstance].settingInfo.parkingMode;
        }];
    }];
}
...
- (NSArray *)rowHeights{
    ...
    if (!_rowHeights) {
        _rowHeights = @[@(62),@(98),@(98),@(98),@(98),@(62),@(62),@(62),@(62),@(62),@(62),@(62),@(62),@(62)];
    }
    return _rowHeights;
}
...
//        _cells = @[self.recordSoundCell,self.motionDetectionCell,self.clipDurationCell,self.gSensorCell,self.LCDLightCell,self.watermarkCell1,self.dateFormatCell,self.exposureCell,self.correctCameraClockCell,self.formatCell,self.networkConfigureCell,self.factoryResetCell,self.helpCell,self.aboatCell,self.DBValueCell];
        _cells = @[self.recordSoundCell,self.parkingModeCell,self.clipDurationCell,self.volumeCell,self.gSensorCell,self.correctCameraClockCell,self.factoryResetCell,self.formatCell,self.networkConfigureCell,self.helpCell,self.aboatCell,self.OTAUpgradeCell];
    }
    return _cells;
}
